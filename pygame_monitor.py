...
class PygameMonitor:
    ...
    def _draw_charts(self):
        ...
        self._draw_histogram_chart(
            ...
            self.charts['perception_dist'], "Perception Distribution"
        )

        # Aggression distribution chart
        self._draw_histogram_chart(
            chart_x, chart_y + 4 * (chart_height + 15), chart_width, chart_height,
            self.charts['aggression_dist'], "Aggression Distribution"
        )
        
        # Daily statistics chart
        self._draw_daily_stats_chart(
            chart_x, chart_y + 5 * (chart_height + 15), chart_width, chart_height,
    ...
    def _draw_histogram_chart(self, x, y, width, height, data, title):
        """Draw a histogram chart for any distribution"""
        # Background
        chart_rect = pygame.Rect(x, y, width, height)
        pygame.draw.rect(self.screen, COLORS['UI_CHART_BG'], chart_rect)
        pygame.draw.rect(self.screen, COLORS['UI_BORDER'], chart_rect, 2)
        ...
        # Use the data passed to this method instead of hardcoded strength
        if not data.values:
            return
        ...
        for value in values:
            bin_idx = min(int((value - min_val) / (max_val - min_val) * num_bins), num_bins - 1)
            bins[bin_idx] += 1
    ...
    def _draw_status(self):
        ...
        # Status background - make it taller to fit more info
        status_rect = pygame.Rect(status_x, status_y, self.grid_size, 100)  # Increased height from 60 to 100
        ...
        # Live statistics in organized rows
        y_offset = 35
        
        # Row 1: Population and Carrots
        ...
        self.screen.blit(pop_surface, (status_x + 10, status_y + y_offset))
        
        # Row 2: Day and Tick counters
        current_day = len(self.sim.day_population)
        current_tick = getattr(self.sim, 'fights_today', 0)  # Using fights_today as tick counter
        tick_text = f"Day: {current_day} | Tick: {current_tick} | Total Ticks: {current_day * self.cfg.T_DAY + current_tick}"
        tick_surface = self.fonts['small'].render(tick_text, True, COLORS['UI_TEXT'])
        self.screen.blit(tick_surface, (status_x + 10, status_y + y_offset + 20))
        
        # Row 3: Today's events
        fights_today = getattr(self.sim, 'fights_today', 0)
        deaths_today = getattr(self.sim, 'deaths_today', 0)
        births_today = getattr(self.sim, 'births_today', 0)
        events_text = f"Today: Fights: {fights_today} | Deaths: {deaths_today} | Births: {births_today}"
        events_surface = self.fonts['small'].render(events_text, True, COLORS['UI_TEXT'])
        self.screen.blit(events_surface, (status_x + 10, status_y + y_offset + 40))
    
    def _draw_live_counters(self):
        """Draw live counters above the grid"""
        counter_x = self.grid_x
        counter_y = 20  # Above the grid
        counter_width = self.grid_size
        counter_height = 40
        
        # Counter background
        counter_rect = pygame.Rect(counter_x, counter_y, counter_width, counter_height)
        pygame.draw.rect(self.screen, COLORS['UI_CHART_BG'], counter_rect)
        pygame.draw.rect(self.screen, COLORS['UI_BORDER'], counter_rect, 2)
        
        # Get current values
        current_day = len(self.sim.day_population)
        current_tick = getattr(self.sim, 'fights_today', 0)
        total_ticks = current_day * self.cfg.T_DAY + current_tick
        population = len(self.sim.agents)
        carrots = int(self.sim.world.carrots.sum())
        
        # Display main counters
        counter_text = f"Day {current_day} | Tick {current_tick} | Total Ticks {total_ticks} | Population {population} | Carrots {carrots}"
        counter_surface = self.fonts['large'].render(counter_text, True, COLORS['UI_TEXT'])
        
        # Center the text
        text_rect = counter_surface.get_rect(center=counter_rect.center)
        self.screen.blit(counter_surface, text_rect)
    
    def _draw_corner_overlay(self):
        """Draw live stats overlay in top-right corner of grid"""
        overlay_x = self.grid_x + self.grid_size - 200
        overlay_y = self.grid_y + 10
        overlay_width = 190
        overlay_height = 80
        ...
        overlay_surface.fill(COLORS['UI_CHART_BG'])
        ...
        # Render text
        day_surface = self.fonts['medium'].render(day_text, True, COLORS['UI_TEXT'])
        tick_surface = self.fonts['medium'].render(tick_text, True, COLORS['UI_TEXT'])
        pop_surface = self.fonts['medium'].render(pop_text, True, COLORS['UI_TEXT'])
        
        # Position text
        self.screen.blit(day_surface, (overlay_x + 10, overlay_y + 10))
        self.screen.blit(tick_surface, (overlay_x + 10, overlay_y + 30))
        self.screen.blit(pop_surface, (overlay_x + 10, overlay_y + 50))
    ...
    def _update_chart_data(self):
        ...
        # Perception distribution (current live data)
        if self.sim.agents:
            perceptions = [agent.genome.perception for agent in self.sim.agents if agent.alive]
            if perceptions:
                self.charts['perception_dist'].values = perceptions
                self.charts['perception_dist'].max_value = max(perceptions)
                self.charts['perception_dist'].min_value = min(perceptions)
        ...
        # Aggression distribution (current live data)
        if self.sim.agents:
            aggressions = [agent.genome.aggression for agent in self.sim.agents if agent.alive]
            if aggressions:
                self.charts['aggression_dist'].values = aggressions
                self.charts['aggression_dist'].max_value = max(aggressions)
                self.charts['aggression_dist'].min_value = min(aggressions)
    ...
    def render(self, day_idx, tick_idx):
        ...
        self._draw_buttons()
        self._draw_status()
        self._draw_live_counters()
        self._draw_corner_overlay()
